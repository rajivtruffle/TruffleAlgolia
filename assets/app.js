// assets/app.js
(function () {
  const REPO = "TruffleAlgolia"; // must match your repo
  const ESW_LANG_MAP = { en: "en_US", fr: "fr", de: "de" }; // ← Salesforce-supported
  ...
  function langUrl(lang) { return getBasePath() + lang + "/"; }
  ...
  window.addEventListener("resize", (() => {
    let t;
    return () => { clearTimeout(t); t = setTimeout(() => (window.TruffleContext.device = deviceType()), 200); };
  })());
  ...
  if (sel) {
    ...
    sel.addEventListener("change", (e) => window.location.assign(langUrl(e.target.value)));
  }
  ...
  (function setLanguageBackground() {
    ...
    const img = new Image();
    img.onload = () => { bg.style.backgroundImage = `url("${url}")`; console.log("🎯 Background set:", url); };
    img.onerror = () => {
      ...
      for (const f of fallbacks) {
        ...
        test.onload = () => { bg.style.backgroundImage = `url("${f}")`; console.log("✅ Background fallback used:", f); };
  ...
  function pushPrechatFields() {
    ...
    } catch (e) { console.warn("Prechat push failed:", e); }
  }
    ...
    try {
      if (window.embeddedservice_bootstrap?.settings) {
        embeddedservice_bootstrap.settings.language =
          ESW_LANG_MAP[currentLang()] || "en_US";
      }
    } catch (e) { console.warn("Could not set ESW language:", e); }
    pushPrechatFields();
  });
  ...
  (function ensureESW() {
    ...
    function setLanguageFromUrl() {
      try {
        const lang = ESW_LANG_MAP[currentLang()] || "en_US";
        embeddedservice_bootstrap.settings.language = lang;
        console.log("🌐 ESW language set to:", lang);
      } catch (e) { console.warn("Could not set ESW language:", e); }
    }
    ...
    const timer = setInterval(() => {
      ...
      if (Date.now() - start > 10000) { clearInterval(timer); console.warn("Timed out waiting for ESW bootstrap."); }
    }, 100);
  })();
})();